...
#endif
#ifdef MODULE_ML7396
void receive_ml7396_packet(ieee802154_packet_t *trans_p);
#endif
...
void transceiver_init(transceiver_type_t t)
{
    ...
    if (t & ~(TRANSCEIVER_CC1100 | TRANSCEIVER_CC2420 | TRANSCEIVER_MC1322X | TRANSCEIVER_NATIVE | TRANSCEIVER_AT86RF231 | TRANSCEIVER_ML7396)) {
...
#endif
#ifdef MODULE_ML7396
    else if (transceivers & TRANSCEIVER_ML7396) {
        DEBUG("transceiver: Transceiver started for ML7396\n");
        ml7396_init(transceiver_pid);
    }
...
static void receive_packet(uint16_t type, uint8_t pos)
{
    ...
    /* copy packet and handle it */
        ...
        else if (type == RCV_PKT_ML7396) {
#ifdef MODULE_ML7396
            ieee802154_packet_t *trans_p = &(transceiver_buffer[transceiver_buffer_pos]);
            receive_ml7396_packet(trans_p);
#endif
        }
...
void receive_ml7396_packet(ieee802154_packet_t *trans_p)
{
    DEBUG("Handling ML7396 packet\n");
    dINT();
    ml7396_packet_t *p = &ml7396_rx_buffer[rx_buffer_pos];
    trans_p->length = p->length;
    trans_p->rssi = p->rssi;
    trans_p->crc = p->crc;
    trans_p->lqi = p->lqi;
    memcpy(&trans_p->frame, &p->frame, sizeof(trans_p->frame));
    memcpy(&data_buffer[transceiver_buffer_pos * ML7396_MAX_DATA_LENGTH], p->frame.payload,
           p->frame.payload_len);
    trans_p->frame.payload = (uint8_t *) & (data_buffer[transceiver_buffer_pos * ML7396_MAX_DATA_LENGTH]);
    trans_p->frame.payload_len = p->frame.payload_len;
    eINT();
...
static int8_t send_packet(transceiver_type_t t, void *pkt)
{
    ...
    switch (t) {
        ...
        case TRANSCEIVER_ML7396:
            memcpy(&ml7396_pkt.frame, &p->frame, sizeof(ieee802154_frame_t));
            ml7396_pkt.length = p->frame.payload_len + IEEE_802154_FCS_LEN;
            res = ml7396_send(&ml7396_pkt);
            break;
#endif
...
static radio_address_t set_address(transceiver_type_t t, void *address)
{
    ...
    switch (t) {
        ...
        case TRANSCEIVER_ML7396:
            return ml7396_set_address(addr);
#endif
...
static transceiver_eui64_t get_long_addr(transceiver_type_t t)
{
    switch (t) {
        ...
        case TRANSCEIVER_ML7396:
            return ml7396_get_address_long();
#endif
...
static transceiver_eui64_t set_long_addr(transceiver_type_t t, void *address)
{
    ...
    switch (t) {
        ...
        case TRANSCEIVER_ML7396:
            return ml7396_set_address_long(addr);
#endif
...
static void set_monitor(transceiver_type_t t, void *mode)
{
    ...
    switch (t) {
        ...
        case TRANSCEIVER_ML7396:
            //ml7396_set_monitor(*((uint8_t *) mode));
            break;
#endif
