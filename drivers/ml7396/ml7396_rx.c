...
void ml7396_rx_handler(uint32_t status)
{
    ...
    phr_len = (int) (NTOHS(*(uint16_t *) tmp) & 0x07ff);
    ...
    ml7396_fifo_read(buf, phr_len + 1);

    /* read ED */
    ml7396_rx_buffer[rx_buffer_next].ed = buf[phr_len + 1];
    ...
    ieee802154_frame_read(&buf[2], &ml7396_rx_buffer[rx_buffer_next].frame,
